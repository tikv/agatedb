...
fn test_concurrent_basic(n: usize, cap: u32, value_len: usize) {
    ...
    let kvs: Vec<_> = (0..n)
        .map(|i| (format!("{:05}", i), format!("{1:00$}", value_len, i)))
        .collect();
    ...
    for (k, v) in kvs.clone() {
        ...
        pool.spawn(move |_: &mut Handle<'_>| {
            list.put(k.as_bytes(), v.as_bytes());
            tx.send(()).unwrap();
        })
    }
    for _ in 0..n {
    ...
    for (k, v) in kvs {
        ...
        let list = list.clone();
        pool.spawn(move |_: &mut Handle<'_>| {
            let val = list.get(k.as_bytes());
            assert_eq!(val, Some(v.as_bytes()), "{}", k);
            tx.send(()).unwrap();
        });
    }
    for _ in 0..n {
...
fn test_iterator_next() {
    ...
    for i in (0..n).rev() {
        let key = format!("{:05}", i);
        list.put(key.as_bytes(), new_value(i).as_slice());
    }
    iter_ref.seek_to_first();
    for i in 0..n {
        assert!(iter_ref.valid());
        let v = iter_ref.value();
        assert_eq!(v, new_value(i).as_slice());
        iter_ref.next();
    }
    assert!(!iter_ref.valid());
}
...
fn test_iterator_seek() {
    ...
    for i in (0..n).rev() {
        let v = i * 10 + 1000;
        let key = format!("{:05}", v);
        list.put(key.as_bytes(), new_value(v).as_slice());
    }
    iter_ref.seek_to_first();
    assert!(iter_ref.valid());
    assert_eq!(iter_ref.value(), b"01000");
